...
BASE_DIR = Path("/Users/wash198/Documents/Projects/Science_Projects/MPII_CMM/LLM_Fine_Tuning/Claude")
...
def convert_geodatabase_to_csv(gdb_path, output_dir):
    ...
    for i, layer in enumerate(layers):
        ...
        try:
            result = subprocess.run(
                ['ogr2ogr', '-f', 'CSV', str(output_file), str(gdb_path), layer,
                 '-lco', 'GEOMETRY=AS_WKT'],
...
def write_schema_markdown(schemas, output_file):
    ...
    with open(output_file, 'w') as f:
        ...
        f.write("This document describes the schema for all CSV data files in the CMM supply chain data collection.\n\n")
        ...
        for category, data in schemas.items():
            ...
            if data['file_count'] > 20:
                f.write("*Note: Showing representative schemas (many files have similar structure)*\n\n")
...
def main():
    ...
    total_rows = sum(
        sum(s.get('row_count', 0) for s in d['schemas'])
