        ...
        for cmm_category, usgs_patterns in CMM_COMMODITY_MAPPING.items():
            ...
            if matching_files:
                ...
                for csv_file in matching_files:
                    ...
                    dest_file = cmm_dir / f"{cmm_category.lower().replace(' ', '_')}_{csv_file.name}"
                    try:
...
Examples:
  ...
  python download_usgs_mcs.py --year 2024 --item-id 65a6e45fd34e5af967a46749
    ...
    parser.add_argument(
        '--year',
        type=int,
        help='Specific year to download (default: all years 2020-2024)'
    )
    
    parser.add_argument(
        '--item-id',
        type=str,
        help='ScienceBase catalog item ID for the year'
    )
