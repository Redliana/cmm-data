...
class USGSMCSIndividualDownloader:
    ...
    def find_cmm_commodity_items(self, parent_id: str, year: Optional[int] = None) -> Dict[str, List[Dict]]:
        """
    ...
    def download_year_individual_commodities(self, year: int, parent_id: str) -> Dict:
        ...
        for cmm_category, items in cmm_items.items():
            ...
            for item in items:
                files = self.download_commodity_item(
                    item['id'],
                    item['usgs_commodity'],
                    year
                )
...
Examples:
  ...
  python download_usgs_mcs_individual.py --year 2022 --release-id <ITEM_ID>
    ...
    parser.add_argument(
        '--parent-id',
        type=str,
        help='ScienceBase parent catalog item ID (e.g., NMIC catalog)'
    )
    ...
    if args.release_id:
        ...
        for child in children:
            title = child.get('title', '').upper()
            if str(args.year) in title and ('MINERAL COMMODITY SUMMARIES' in title or 'MCS' in title):
