        ...
        for flow in flows_to_query:
            try:
                ...
                if flow_code not in ['M', 'X']:
                    raise ValueError(f"Invalid flow code: {flow}. Must be 'M' (import) or 'X' (export)")
...
Examples:
  ...
  python un_comtrade_query.py --list-commodities
    ...
    parser.add_argument(
        '--commodity',
        type=str,
        help='HS commodity code (e.g., 2846 for REE compounds)'
    )
    
    parser.add_argument(
        '--commodity-name',
        type=str,
        help='CMM commodity name (e.g., REE_compounds, Lithium_ores)'
    )
    
    parser.add_argument(
        '--year',
        type=int,
        help='Year of trade data (e.g., 2023)'
    )
    ...
    parser.add_argument(
        '--output',
        type=str,
        help='Output file path (CSV, JSON, or Excel format)'
    )
