            ...
            for reporter, partner in countries:
                for year in years:
                    for flow_code in flows_to_query:
                        ...
                        query_id = f"{commodity_name}_{hs_code}_{reporter}_{partner}_{year}_{flow_code}"
                        ...
                        if self._check_data_exists(existing_df, hs_code, reporter, partner, year, flow_code):
                            skipped_count += 1
                            continue
                    
                        try:
                            print(f"  Querying: {hs_code} | {reporter} -> {partner} | {year} | {flow_code}")
        ...
        if all_results:
            ...
            if not existing_df.empty:
                ...
                return {
                    ...
                    'new_records': 0,
                    'filepath': str(self.output_dir / f"{commodity_name.lower().replace(' ', '_')}_trade_data.csv"),
                    'queries_executed': 0,
                    'skipped': skipped_count,
...
Examples:
  ...
  python pull_gold_qa_data.py --api-key YOUR_KEY --output-dir gold_qa_data
    ...
    parser.add_argument(
        '--api-key',
        type=str,
        help='UN Comtrade API key (or set UN_COMTRADE_API_KEY env var)'
    )
